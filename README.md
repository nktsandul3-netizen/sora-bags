...
## Instagram на главной

Блок «Мы в Instagram» показывает **ручной список постов** `@sora.italy` — без Instagram API и без токенов Meta.
Каждая карточка ведёт на конкретный пост или reel в Instagram.

### Как добавить или обновить посты

1. Откройте `src/lib/instagram.ts` и добавьте/измените записи в `manualInstagramPosts` (ссылка + id).
2. Скачайте превью в `public/instagram/`:

```bash
node scripts/fetch-instagram-thumbnails.mjs
```

3. Перезапустите `npm run dev` и откройте главную страницу.
...
- `node scripts/fetch-instagram-thumbnails.mjs` — обновить превью постов Instagram
